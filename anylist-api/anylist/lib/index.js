...
class AnyList extends EventEmitter {
  constructor({
    email,
    password,
    credentialsFile = path.join(os.homedir(), ".anylist_credentials"),
  }) {
    super();
      ...
      hooks: {
        beforeError: [
          (error) => {
            ...
            console.error(
              `Endpoint ${url} returned uncaught status code ${response.statusCode}`,
            );
            return error;
          },
        ],
      ...
      hooks: {
        ...
        afterResponse: [
          async (response, retryWithMergedOptions) => {
            ...
            const url = response.request.options.url.href;
            console.info(
              `Endpoint ${url} returned status code 401, refreshing access token before retrying`,
            );
  ...
  async login(connectWebSocket = true) {
    ...
    if (!this.accessToken || !this.refreshToken) {
      console.info(
        "No saved tokens found, fetching new tokens using credentials",
      );
  ...
  async _loadCredentials() {
    ...
    if (!fs.existsSync(this.credentialsFile)) {
      console.info(
        "Credentials file does not exist, not loading saved credentials",
      );
      return;
    }
  ...
  _setupWebSocket() {
    ...
    this.ws = new WebSocket(
      "wss://www.anylist.com/data/add-user-listener",
      [],
      {
  ...
  async getLists(refreshCache = true) {
    ...
    // console.log("LISTS: ", decoded.shoppingListsResponse.newLists);
    this.lists = decoded.shoppingListsResponse.newLists.map(
      // (list) => console.log("LIST: ", list),
      (list) => new List(list, this),
    );

    for (const response of decoded.starterListsResponse.recentItemListsResponse
      .listResponses) {
      const list = response.starterList;
      this.recentItems[list.listId] = list.items.map(
        (item) => new Item(item, this),
      );
    }

    let favoriteLists =
      decoded.starterListsResponse.favoriteItemListsResponse.listResponses.map(
        (obj) => obj.starterList,
      );

    // console.log("FAVORITES: ", favoriteLists);
    console.log("loading favorite lists items");
    this.favoriteItems = favoriteLists.map(
      // (list) => console.log("FAVS: ", list),
      (list) => new List(list, true, this),
    );

    // for (const response of decoded.starterListsResponse
    //   .favoriteItemListsResponse.listResponses) {
    //   const list = response.starterList;
    //   this.favoriteItems[list.listId] = list.items.map(
    //     (item) => new Item(item, this),
    //   );
    // }
  ...
  getFavoriteItemsByListId(identifier) {
    // console.log(identifier, this.favoriteItems);
    return this.favoriteItems.find((l) => l.identifier === identifier);
  }
  ...
  async getMealPlanningCalendarEvents(refreshCache = true) {
    ...
    this.mealPlanningCalendarEvents =
      decoded.mealPlanningCalendarResponse.events.map(
        (event) => new MealPlanningCalendarEvent(event, this),
      );

    // Map and assign labels
    this.mealPlanningCalendarEventLabels =
      decoded.mealPlanningCalendarResponse.labels.map(
    ...
    this.recipes = decoded.recipeDataResponse.recipes.map(
      (recipe) => new Recipe(recipe, this),
    );
    for (const event of this.mealPlanningCalendarEvents) {
      event.recipe = this.recipes.find(
        (recipe) => recipe.identifier === event.recipeId,
      );
    }
